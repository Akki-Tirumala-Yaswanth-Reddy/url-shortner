...
| No click analytics | `click_count` and `last_accessed_at` updated atomically on every redirect |
| No health or readiness endpoints | `GET /healthCheck` (liveness) and `GET /readyCheck` (DB readiness) added |
